...
public class DataSheetGraph extends JPanel {
    ...
    public void setDataSheet(DataSheet dataSheet) {
        this.dataSheet = dataSheet; repaint();
    }
...
// Определяем минимальное значение Х
        if (dataSheet != null) {
            ...
            for (int i = 0; i < size; i++)
                if (dataSheet.getDateItem(i).getX() < result) result = dataSheet.getDateItem(i).getX();
        }
        return result;
    }
    ...
    private double maxX() {
        double result = 0;
// Определяем максимальное значение Х
        if (dataSheet != null) {
            int size = dataSheet.size();
            for (int i = 0; i < size; i++)
                if (dataSheet.getDateItem(i).getX() > result) result = dataSheet.getDateItem(i).getX();
...
// Определяем минимальное значение Y
        if (dataSheet != null) {
            int size = dataSheet.size();
            for (int i = 0; i < size; i++)
                if (dataSheet.getDateItem(i).getY() < result) result = dataSheet.getDateItem(i).getY();
        }
        return result;
    }
...
// Вообще-то следует создать метод для вычисления шага сетки
        ...
        for (double dx = -xStep; dx >= xMin; dx -= xStep) {
            ...
            gr.setPaint(Color.LIGHT_GRAY);
            gr.draw(new Line2D.Double(x, 0, x, height));
            gr.setPaint(Color.BLACK); gr.drawString(Math.round(dx/xStep)*xStep+"", (int)x+2, 10);
        }
        ...
        if (dataSheet != null) {
            if (!isConnected) {
                for (int i = 0; i < dataSheet.size(); i++) {
                    double x = x0 + (dataSheet.getDateItem(i).getX() * xScale);
                    double y = y0 - (dataSheet.getDateItem(i).getY() * yScale);
                ...
                double xOld = x0 + dataSheet.getDateItem(0).getX() * xScale;
                double yOld = y0 - dataSheet.getDateItem(0).getY() * yScale;
